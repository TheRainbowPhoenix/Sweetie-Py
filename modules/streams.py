...
class Streams:
    ...
    def __init__(self, bot):
        ...
        self.piczel_streams = dataIO.load_json("data/streams/piczel.json")
    ...
    @commands.command()
    async def piczel(self, stream: str):
        """Checks if piczel stream is online"""
        stream = escape_mass_mentions(stream)
        regex = r'^(https?\:\/\/)?(www\.)?(piczel\.tv\/)'
        stream = re.sub(regex, '', stream)
        try:
            embed = await self.piczel_online(stream)
        except OfflineStream:
            await self.bot.say(stream + " is offline.")
        except StreamNotFound:
            await self.bot.say("That stream doesn't exist.")
            ...
            await self.bot.say("HTTPException.")
        else:
            await self.bot.say(embed=embed)
    ...
    @streamalert.command(name="piczel", pass_context=True)
    async def piczel_alert(self, ctx, stream: str):
        """Adds/removes piczel alerts from the current channel"""
        stream = escape_mass_mentions(stream)
        regex = r'^(https?\:\/\/)?(www\.)?(piczel\.tv\/)'
        stream = re.sub(regex, '', stream)
        channel = ctx.message.channel
        try:
            await self.piczel_online(stream)
        except StreamNotFound:
            await self.bot.say("That stream doesn't exist.")
            return
        except APIError:
        ...
        enabled = self.enable_or_disable_if_active(self.piczel_streams,
                                                   stream,
                                                   channel)
        ...
        dataIO.save_json("data/streams/piczel.json", self.piczel_streams)
    ...
    async def piczel_online(self, user):
        url = "https://piczel.tv/api/streams/" + user

        async with aiohttp.get(url) as r:
            data = await r.text(encoding='utf-8')
        if r.status == 200:
            data = json.loads(data)
            if data['data'][0]['live'] is True:
                return self.piczel_embed(data['data'][0])
            else:
                raise OfflineStream()
        elif r.status == 404:
            raise StreamNotFound()
    ...
    def piczel_embed(self, data):
        avatar = data['user']['avatar']['avatar']['url']
        url = "https://piczel.tv/watch/" + data["slug"]
        thumbnail = ("https://piczel.tv/api/screenshots/stream_{}.jpg"
                     "".format(data['id']))
        embed = discord.Embed(title=data["title"], url=url)
        embed.set_author(name=data["username"])
        embed.set_image(url=thumbnail + self.rnd_attr())
        embed.add_field(name="Followers", value=data["follower_count"])
        embed.add_field(name="Total views", value=data["viewers"])
        embed.set_thumbnail(url=avatar)
        embed.color = 0x969696
	
        mdatas=[]
        for i in data['tags']:
            mdatas.append(i['title'])
        mdatas = ", ".join(mdatas)
        ...
        embed.set_footer(text="{}{} | Tags: {}"
                              "".format(data['adult'], data['description'], mdatas))
        return embed
    ...
    async def stream_checker(self):
        ...
        while self == self.bot.get_cog("Streams"):
            ...
            streams = ((self.twitch_streams,  self.twitch_online),
					   ...
					   (self.piczel_streams, self.piczel_online))
